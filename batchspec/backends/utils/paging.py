...
class PageManager:
    ...
    *Backward-compatible wrappers*:
    ...
    def allocate_counts(
        ...
        """Add pages to requests (vectorized one-shot operation).
    ...
    def allocate(
        ...
        """Backward-compatible wrapper: allocate pages by repeated request indices.
        
        Args:
            requested_indices: Request indices (duplicates = multiple pages)
            kv_page_indices: Current page indices or None
            kv_page_indptr: Current page indptr or None
            
        Returns:
            Same as allocate_counts
        """
        device = self.num_allocated_pages.device
        requested_indices = requested_indices.to(device=device, dtype=torch.int32)
        add_counts = torch.bincount(requested_indices, minlength=self.bsz).to(torch.int32)
        return self.allocate_counts(add_counts, kv_page_indices, kv_page_indptr)
    ...
    def free(
        ...
        """Backward-compatible wrapper: free pages by repeated request indices.
        ...
        device = self.num_allocated_pages.device
        requested_indices = requested_indices.to(device=device, dtype=torch.int32)
        remove_counts = torch.bincount(requested_indices, minlength=self.bsz).to(torch.int32)
        return self.free_counts(remove_counts, kv_page_indices, kv_page_indptr)
