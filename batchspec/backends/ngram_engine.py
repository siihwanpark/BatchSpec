...
class NGramDraftEngine(BaseEngine):
    ...
    def generate_batch(self, input_ids, full_input_ids, max_gen_len: int, prefix_len: int):
        ...
        output = torch.zeros(bsz, max_gen_len * 2, device=device, dtype=torch.long)
        ...
        while num_generated_tokens.mean(dtype=torch.float32) < max_gen_len and not terminal:
            with profiler.step_timing_ctx():
                ...
                # N-gram Draft
                full_sequence = torch.cat([full_input_ids, output[:, :1 + num_generated_tokens.max()]], dim=1)
                tokens_buffer[:, 1:] = self.draft(full_sequence, max_ngram_size, prefix_len + 1 + num_generated_tokens)
    ...
    def evaluate_posterior(
        ...
        Args:
            ...
            target_preds_or_logits (torch.Tensor):
                ...
                - (Sampling) The predicted logits by the target model. Shape: [bsz, draft_length + 1, vocab_size]
        ...
        if self.greedy:
            ...
            # Perform the chain speculative sampling
