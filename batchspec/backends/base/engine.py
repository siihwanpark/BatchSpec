...
class BaseEngine(AttentionWrapperMixin):
    ...
    def sampling_fault_handler(
        self,
        tokens_buffer: Tensor,
        bonus_tokens: Tensor,
        accept_nums: Tensor,
    ) -> tuple[Tensor, Tensor]:
        """
        Handle rare sampling faults where FlashInfer's `chain_speculative_sampling`
        produces an invalid bonus token (e.g, 1017328159).

        If a bonus token is outside the valid vocabulary range, it is replaced with
        the first token in `tokens_buffer`, and the corresponding `accept_nums` is
        decremented. This indicates that no draft tokens nor bonus tokens are accepted,
        resulting in an empty accepted sequence.

        The fallback token is required to proceed to the next step (e.g., feeding a
        token to the drafter), so we reuse the last valid token generated by the target.
        
        Args:
            tokens_buffer: [bsz, k+1] buffer of target-generated token ids
            bonus_tokens: [bsz, 1] bonus token ids from speculative sampling
            accept_nums:  [bsz] number of accepted tokens per sequence

        Returns:
            updated_bonus_tokens: [bsz, 1] corrected bonus token ids
            updated_accept_nums:  [bsz] corrected accept counts
        """
        if hasattr(self.model, 'eagle'):
            vocab_size = self.model.eagle.config.draft_vocab_size
        else:
            vocab_size = self.model.config.vocab_size
        
        invalid_mask = (bonus_tokens[:, 0] < 0) | (bonus_tokens[:, 0] >= vocab_size)
        if invalid_mask.any():
            bonus_tokens[invalid_mask, 0] = tokens_buffer[invalid_mask, 0].to(bonus_tokens.dtype)
            accept_nums[invalid_mask] -= 1

        return bonus_tokens, accept_nums
