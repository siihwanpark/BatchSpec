                ...
                for backend in eagle magicdec standalone; do
                    if [ $backend == "standalone" ]; then
                        if [ $prefix_profile == "16k" ]; then
                            prefix_profile="8k"
                        elif [ $prefix_profile == "32k" ]; then
                            prefix_profile="16k"
                        fi
                    fi
                    append_row "4g" "$model" "$backend" 1 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
                    append_row "4g" "$model" "$backend" 2 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
        ...
        for mode in sampling; do
            for bsz in "${bsz_list[@]}"; do
                ...
                for backend in eagle magicdec standalone; do
                    ...
                    append_row "2g0" "$model" "$backend" 1 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
                    append_row "2g1" "$model" "$backend" 2 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
        ...
        for mode in sampling; do
            for bsz in "${bsz_list[@]}"; do
                ...
                for backend in eagle magicdec standalone; do
                    ...
                    append_row "1g0" "$model" "$backend" 1 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
                    append_row "1g1" "$model" "$backend" 2 "$dataset" "$mode" "$bsz" "$prefix_profile" 6
