...
for model in Qwen3-8B DSL-8B;do
	if [ $model == "Qwen3-8B" ]; then
		extra_args=(--top_k 20)
		mtp_args=(--lora_checkpoint_path $base_ckpt_dir/MTP_adapters/Qwen3-8B/model.pth --lora_rank 16 --lora_alpha 32)
	else
		extra_args=()
		mtp_args=(--lora_checkpoint_path $base_ckpt_dir/MTP_adapters/DeepSeek-R1-Distill-Llama-8B/model.pth --lora_rank 16 --lora_alpha 32)
	fi
